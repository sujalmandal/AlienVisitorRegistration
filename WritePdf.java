...
public class WritePdf 
{
  ...
  private PdfPCell cell; 				//Global declarations here
  private String file =null; 
  private Font bigFont = new Font(Font.FontFamily.TIMES_ROMAN, 18,Font.BOLD);	//bigFont defined here

  public void writeToPdf(Details alienDetail) // the main method which prints the data to the PDF file
  {
	  try
	    {
		  file="c:/"+alienDetail.getCodeName()+".pdf";
	      document = new Document();	//create a new document object
	      PdfWriter.getInstance(document, new FileOutputStream(file));
	      document.open();		        //open the document
	      addMetaData(document);        // add all metadata to the pdf file.
	      addContents(document, alienDetail);// add the registration page
  ...
  private void addMetaData(Document document)   //Add metadata to the pdf file to be written
  {
  ...
  private void addContents(Document document,Details alienDetail) throws DocumentException 
  {
  ...
  private void createTable(Document doc,Details alienDetail) throws BadElementException 
  {
  ...
  private void addTableCell(PdfPTable table,String content )  ///method to write cells to the table
  {
